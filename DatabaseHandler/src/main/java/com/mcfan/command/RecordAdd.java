...
public class RecordAdd  implements CommandExecutor {
		...
		if(length >= 3) {
			...
			if(length==3) {
				new QueryHandler() {
					...
					@Override
					public void query() {
						getSqlQueries().addValeue(tableName, columnName, value);
					}
				};
				return true;
				...
				new QueryHandler() {
					...
					@Override
					public void query() {
						getSqlQueries().addValeue(tableName, columnName, value, whereStatment.toString());
					}
				};
				return true;
			}
